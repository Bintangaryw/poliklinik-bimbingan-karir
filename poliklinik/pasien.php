...
if (isset($_POST['simpan'])) {
    if (isset($_POST['id'])) {
        ...
        // Menggunakan format tahun-bulan-id untuk no_rm
        $tahunBulan = date("Ym");
        $result = mysqli_query($mysqli, "SELECT MAX(id) AS maxId FROM pasien");
        $data = mysqli_fetch_assoc($result);
        $maxId = $data['maxId'];
        $newId = $maxId + 1;
        $no_rm = $tahunBulan . "-" . str_pad($newId, 3, '0', STR_PAD_LEFT);
...
<div class="container">
    ...
    <form class="form row" method="POST" action="" name="myForm" onsubmit="return(validate());">
        ...
        <div class="row">
            ...
            <div>
                <input type="text" class="form-control" name="nama_poli" id="inputNama" placeholder="Nama" value="<?php echo $nama ?>">
            </div>
        </div>
        <div class="row mt-1">
            ...
            <div>
                <input type="text" class="form-control" name="keterangan" id="inputAlamat" placeholder="Alamat" value="<?php echo $alamat ?>">
            </div>
        </div>
        <div class="row mt-1">
